...
export default async function LeaveRequestPage({ params, searchParams }: PageProps) {
  ...
  const approveAction = decideRequest.bind(null, "leave", id, "approved");
  const rejectAction = decideRequest.bind(null, "leave", id, "rejected");
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <header className="bg-white p-6 rounded-2xl shadow-md border border-slate-100 mb-6">
          <div className="flex items-start justify-between gap-3 mb-3">
            <div>
              <div className="text-xs text-slate-500 font-cairo mb-1">طلب إجازة</div>
          ...
          <div className="grid grid-cols-2 gap-3 mt-4 text-sm font-cairo">
            <Field label="نوع الإجازة" value={LEAVE_TYPE_LABELS_AR[row.leave_type]} />
            <Field label="عدد الأيام" value={`${row.days_count} يوم`} />
            <Field label="من" value={row.start_date} />
            <Field label="إلى" value={row.end_date} />
          </div>
        ...
        {row.hr_notes && (
          <div className="mb-6 p-4 rounded-xl bg-cyan-50 border border-cyan-200">
            <div className="text-xs font-bold text-cyan-800 mb-1 font-cairo">
              ملاحظات HR (سُجِّلت عند المراجعة)
            </div>
            <p className="text-sm text-slate-800 font-cairo whitespace-pre-line">
              {row.hr_notes}
            </p>
        ...
        {isPending && (
          <div className="bg-white p-6 rounded-2xl shadow-md border border-slate-100">
            ...
            {/* Single form, two formAction targets so both buttons share the
                same hr_notes input. Avoids React's "form per action" duplication. */}
            <form className="space-y-4">
              <div>
                <label htmlFor="hr_notes" className="block text-sm font-medium text-slate-700 mb-2 font-cairo">
                  ملاحظات (اختياري)
                </label>
                <textarea
                  id="hr_notes"
                  rows={3}
                  name="hr_notes"
                  placeholder="مثلًا: موافق بشرط ترتيب التسليم..."
