...
//
// PROVIDER CHOICE — Pollinations.ai:
//   We use https://image.pollinations.ai/ (FLUX-based, free, no API key)
//   over Imagen 3 / DALL-E for a few practical reasons:
//
//   1) FREE forever — no quota chip eating $0.04 per post. Nidham is
//      bootstrapped; predictable $0 image-gen cost matters more than
//      the marginal quality gain from Imagen 3.
//   2) No new API key to provision — keeps the env-var surface area
//      small for Basem (he's already juggling Groq + Gemini + Meta).
//   3) Returns image bytes directly via GET, no async polling like
//      DALL-E batch jobs.
//
//   If quality becomes a blocker we can swap to google.image('imagen-3')
//   later — the storage / upload flow stays identical, only the bytes
//   source changes.
//
// FLOW:
...
// Pollinations.ai endpoint. Defaults:
//   - model: flux       (best quality balance; "flux-realism" is too literal)
//   - nologo: true      (clean image, no watermark)
//   - enhance: false    (don't auto-rewrite the prompt; we already engineered it)
//   - safe: true        (NSFW filter on)
...
export async function generateImageBytes(args: {
  ...
  // Map aspect → width/height. Pollinations accepts arbitrary sizes; we
  // use round numbers that FB / IG / TG all accept without re-encode.
  const dims =
  ...
  // Generous timeout — Pollinations can take 20-40s under load.
  ...
  try {
    ...
    if (buf.length < 1000) {
      // Pollinations sometimes returns a 1-pixel placeholder on failure.
      // Treat tiny responses as failures so we can retry / surface.
      throw new Error(
